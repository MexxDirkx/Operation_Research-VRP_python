...
def SA_between_all_routes(routes, coords, depot_idx=0,
                          T0=500.0, alpha=0.995, iters=2000, min_T=1e-3):
    """
    Simulated annealing OVER MULTIPLE ROUTES.
    De functie wisselt willekeurige klantsegmenten (of 1 klant)
    tussen verschillende routes.

    INPUT:
        routes: lijst van routes, bv:
                [[0, 3, 8, 4], [0, 7, 2, 10], [0, 9, 5]]
        coords: lijst met (x,y)'s
    OUTPUT:
        nieuwe lijst met routes (zelfde structuur) 
    """
    ...
    while T > min_T:
        ...
        for _ in range(iters):
            ...
            # -----------------------------
            # 1. Kies twee verschillende routes
            # -----------------------------
            if len(curr_routes) < 2:
                break

            r1, r2 = random.sample(range(len(curr_routes)), 2)
            routeA = curr_routes[r1]
            routeB = curr_routes[r2]

            # routes moeten minstens 2 klanten hebben (0 is depot)
            if len(routeA) <= 2 or len(routeB) <= 2:
                continue

            # -----------------------------
            # 2. Kies segmentlengte voor de swap
            # -----------------------------
            lenA = len(routeA) - 1
            lenB = len(routeB) - 1

            segA_len = random.randint(1, min(3, lenA - 1))
            segB_len = random.randint(1, min(3, lenB - 1))

            startA = random.randint(1, lenA - segA_len)
            startB = random.randint(1, lenB - segB_len)

            segA = routeA[startA:startA + segA_len]
            segB = routeB[startB:startB + segB_len]

            # -----------------------------
            # 3. Maak nieuwe routes (deepcopy, niet in-place!)
            # -----------------------------
            newA = routeA[:startA] + segB + routeA[startA + segA_len:]
            newB = routeB[:startB] + segA + routeB[startB + segB_len:]

            # depot moet altijd op index 0 staan
            if newA[0] != depot_idx or newB[0] != depot_idx:
                continue
